...
_mcp_tools_cache: list[BaseTool] | None = None#缓存的mcp工具列表
_cache_initialized = False#是否初始化缓存
...
def _is_cache_stale() -> bool:
    ...
    current_mtime = _get_config_mtime()# 获取当前配置文件的修改时间
    ...
    if current_mtime > _config_mtime:# 如果当前配置文件的修改时间大于缓存时的修改时间
...
def reset_mcp_tools_cache() -> None:
    ...
    将缓存的MCP工具列表重置为None，将缓存初始化标志重置为False，将配置文件修改时间重置为None。
